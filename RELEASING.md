...
`github:mat3ra/move#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/move/releases/download/wip-<short-sha>/move.tgz
```
...
```bash
npm run mat3ra:install -- move wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/move@<release-url>` with
